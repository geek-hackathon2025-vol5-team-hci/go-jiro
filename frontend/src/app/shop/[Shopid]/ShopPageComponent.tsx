...
export default function ShopPageComponent({ shop, shopId }: ShopPageComponentProps) {
  ...
  const [selections, setSelections] = useState<Record<string, string>>(() => {
    const allCategories = [...new Set([...editor.savedTicketCategories, ...editor.savedToppingCategories])];
  ...
  useEffect(() => {
    ...
    const getCallText = (category: string) => {
      ...
      const rule = rulesToUse.find(r => r.category === category && r.option === option);
