                ...
                >
                    ...
                    <div className="flex-grow">
                        ...
                        <p className="text-xs text-gray-500">
                            二郎度: {(shop.jiro_difficulty ?? -1) < 0 ? "不明" : shop.jiro_difficulty}
                        </p>
                        <span
                            className="text-xs px-2 py-1 rounded-full text-white font-semibold"
                            style={{ backgroundColor: getColorByScore(shop.jiro_difficulty) }}
                        >
      ...
      {shops.map((shop) => {
        ...
        return (
          <AdvancedMarker
            ...
            title={`${shop.name} - 次郎度: ${getLevelByScore(score)}`}
            onClick={() => onMarkerClick(shop)}
            style={{ opacity: shop.isOpen ? 1 : 0.6 }}
          >
              <Pin
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          setPosition({ lat: 43.0618, lng: 141.3545 }); // 札幌をデフォルトに
        }
      );
    } else {
  ...
  const processedAndSortedShops = useMemo(() => {
    ...
    return [...processed].sort((a, b) => {
      switch (sortType) {
        ...
        case "jiro_high":
          return (b.jiro_difficulty ?? -1) - (a.jiro_difficulty ?? -1);
        case "jiro_low":
          return (a.jiro_difficulty ?? -1) - (b.jiro_difficulty ?? -1);
        default:
          return 0;
      }
    });
      ...
      <main className="flex-grow h-full">
        {!isMenuOpen && (
          <button
            ...
            className="fixed top-20 left-4 z-50 bg-white p-2 rounded-md shadow-lg md:hidden"
          >
            <HamburgerIcon />
          </button>
        )}
        {errorMsg && (
          <div className="absolute top-16 left-0 bg-red-500 text-white p-2 z-10">
            {errorMsg}
          </div>
        )}
        <Map
